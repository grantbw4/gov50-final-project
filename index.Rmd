...
combined %>% filter(grepl(listofgenres[i], genres)) %>% ggplot(aes(x = audience_rating, y = Adjusted_Lifetime_Gross)) + geom_point() + geom_smooth() + labs(title = sprintf("Relationship Between Audience Score and Earnings for %s Movies", listofgenres[i]), x = "Audience Score", y = "Earnings Adjusted for Inflation (2022 Ticket Prices)") -> plot
...
<p> From these graphs, it is clear that some genres, like comedies and action movies, contain a sizable number of movies that perform well at the box office despite lackluster reviews. Other genres, like horror, in particular, appear to have fairly consistent earnings independent from reviews. <p>
...
<p> Lastly, I conducted one last multivariate linear regression to determine which covariates (MPAA rating, reviews, genre, etc) were most predictive of adjusted lifetime earnings. <p>
